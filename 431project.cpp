...
void initbaseline(std::string configuration){
  ...
  GLOB_baseline_EP_pair.second=calculategeomeanExecutionTime(configuration);
}
...
int main(int argc, char** argv){
  ...
  if(2!=argc){
    ...
    if(!(isEarg||isParg)){
      fprintf(stderr,"Invalid argument! Run as DSE e or DSE p for energy or performance run, respectively\n");
      return -1;
    } else {
      system("mkdir -p logs");
  ...
  logfile << calculategeomeanEDP(currentConfiguration)/GLOB_baseline_EP_pair.first << "," 
          << calculategeomeanExecutionTime(currentConfiguration)/GLOB_baseline_EP_pair.second << ","
          << calculategeomeanEDP(currentConfiguration) << "," 
          << calculategeomeanExecutionTime(currentConfiguration) << std::endl;
  ...
  for(unsigned int round =1; round < 51; ++round){
    ...
    for(unsigned int iteration=1; iteration < 21; ++iteration){
      ...
      double proposedGEOEXEC=calculategeomeanExecutionTime(nextconf);
      double bestExec=calculategeomeanExecutionTime(bestTimeconfig);
